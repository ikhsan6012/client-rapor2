...
const PenambahanWPBaru = () => {
  const classes = useStyles()
  const [data, setData] = useState([])
  const [updateInfo, setUpdateInfo] = useState(null)

  useEffect(() => {
    getRequest('persil', { t: 2 })
      .then(res => {
        setData(mutatePenerimaan(res.penambahanWp, 'penambahanWp'))
        setUpdateInfo(res.updateInfo.penambahanWp)
      })
  }, [])
  ...
  return(
    <Container style={{ height: 'calc(100vh - 113px)', overflowY: 'auto' }}>
      <Grid item xs={ 12 }>
        <Grid container>
          <Grid item xs={ 6 }>
            <Typography component="span" variant="h5">Penambahan WP Baru</Typography>
          </Grid>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell className={ classes.bold } colSpan="4" align="center">Jenis WP</TableCell>
              </TableRow>
              <TableRow>
                <TableCell className={ classes.bold } align="center">Badan</TableCell>
                <TableCell className={ classes.bold } align="center">OP_K</TableCell>
